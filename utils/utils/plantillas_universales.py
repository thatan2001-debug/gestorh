...
def _crear_documento_base(ruta_salida: str, datos_empresa: dict, disenio: int = 1,
    ...
    doc = SimpleDocTemplate(
        ruta_salida,
        pagesize=A4,  # 595 x 842 pts — estándar internacional
    ...
    # Pie unificado (mismo estilo que documentos premium):
    # "Gestor RH IA · dd/mm/aaaa" a la izq + "Página X" a la der
    from utils.estilos_corporativos import crear_funcion_pie
    pie_fn = crear_funcion_pie(
        paleta, logo_path=logo_path if usar_marca_agua else None,
...
def _agregar_encabezado(elementos, estilos, datos_empresa: dict, titulo: str,
    ...
    """Agrega encabezado corporativo unificado (mismo estilo que documentos premium).

    Usa crear_encabezado_corporativo de estilos_corporativos para garantizar
    que TODOS los documentos generados por la app se vean iguales:
    - Nombre empresa a la izquierda
    - Logo a la derecha
    - NIT + CIUDAD + Tel + correo debajo del nombre
    - Línea corporativa
    - Título centrado en color primario
    - Fecha + ciudad debajo del título
    """
    from utils.estilos_corporativos import (
        crear_encabezado_corporativo, crear_titulo_documento,
        PALETAS as _P, crear_estilos as _crear_est,
    )
    from reportlab.lib.styles import ParagraphStyle
    from reportlab.lib.enums import TA_LEFT

    # Determinar paleta (el motor universal ya recibió el disenio en la paleta actual)
    paleta = estilos.get("_paleta") or _P.get(1)
    est_unificados = _crear_est(paleta, perfil="compacto")

    # Encabezado empresa + logo (mismo diseño que documentos premium)
    elementos.extend(crear_encabezado_corporativo(datos_empresa, paleta, perfil="compacto"))

    # Título del documento
    elementos.append(crear_titulo_documento(titulo, est_unificados))
    ...
    estilo_fecha = ParagraphStyle(
        "FechaUniversal", parent=est_unificados["cuerpo"],
        fontSize=10.5, leading=14, alignment=TA_LEFT, spaceAfter=12,
    )
    if ciudad:
        elementos.append(Paragraph(f"{ciudad}, {fecha_txt}", estilo_fecha))
    else:
        elementos.append(Paragraph(fecha_txt, estilo_fecha))

    if consecutivo:
        elementos.append(Paragraph(
            f"Consecutivo: <b>{consecutivo}</b>",
            est_unificados["cuerpo"]
        ))
...
def _agregar_firma(elementos, estilos, datos_empresa: dict, empleado: dict = None,
    ...
    # Estilos para celdas de firmas (evitan que HTML aparezca crudo)
    estilo_linea = ParagraphStyle(
        "FirmaLinea", parent=estilos["cuerpo"], alignment=TA_CENTER,
        fontSize=10, leading=12, spaceBefore=0, spaceAfter=0,
    )
    estilo_nombre = ParagraphStyle(
        "FirmaNombre", parent=estilos["cuerpo"], alignment=TA_CENTER,
        fontSize=10, leading=12, fontName="Helvetica-Bold",
        spaceBefore=2, spaceAfter=0,
    )
    estilo_cargo = ParagraphStyle(
        "FirmaCargo", parent=estilos["cuerpo"], alignment=TA_CENTER,
        fontSize=9, leading=11, textColor=colors.HexColor("#6B7280"),
        spaceBefore=0, spaceAfter=0,
    )

    if not doble_firma:
        # Firma simple del representante (alineada a la izquierda estilo carta)
        elementos.append(Paragraph("_" * 45, estilos["cuerpo"]))
        elementos.append(Paragraph(f"<b>{firmante_nombre}</b>", estilo_nombre))
        elementos.append(Paragraph(firmante_cargo, estilo_cargo))
        elementos.append(Paragraph(empresa_nombre, estilo_cargo))
    else:
        # Doble firma: empresa | empleado
        # IMPORTANTE: cada celda debe ser un Paragraph para que se procese HTML.
        # Antes se pasaban strings crudos y aparecía "<b>NOMBRE</b>" como texto.
        nombre_emp = empleado.get("nombre", "") if empleado else ""
        doc_emp = empleado.get("documento", "") if empleado else ""

        tabla_firmas = Table([
            [Paragraph("_" * 30, estilo_linea),
             Paragraph("_" * 30, estilo_linea)],
            [Paragraph(f"<b>{firmante_nombre}</b>", estilo_nombre),
             Paragraph(f"<b>{nombre_emp}</b>", estilo_nombre)],
            [Paragraph(firmante_cargo, estilo_cargo),
             Paragraph(f"CC {doc_emp}", estilo_cargo)],
            [Paragraph(empresa_nombre, estilo_cargo),
             Paragraph("Empleado", estilo_cargo)],
        ], colWidths=[8 * cm, 8 * cm])
...
def _plantilla_llamado_atencion(elementos, estilos, empleado, datos_empresa, extra):
    ...
    Estructura corporativa compacta - todo en una hoja.
    """
    from reportlab.lib.styles import ParagraphStyle
    ...
    # Estilos compactos específicos para el llamado
    ...
    # Asunto
    elementos.append(Paragraph(
        f"<b>Asunto:</b> Llamado de atención escrito - {motivo}",
        estilo_asunto
    ))
    ...
    # Hechos (destacados con indentación)
    if hechos:
        elementos.append(Paragraph(f"<b>Hechos:</b> {hechos}", estilo_hechos))
    ...
    # Consecuencias
    elementos.append(Paragraph(consecuencias, estilo_carta))
...
def generar_documento(
    ...
    # ═══════════════════════════════════════════════════════════════════
    # REDIRECCIÓN AL MOTOR PREMIUM (para unificar diseño de TODOS los docs)
    # Los documentos disciplinarios ahora usan el mismo motor visual
    # que certificados y actas, para consistencia total.
    # ═══════════════════════════════════════════════════════════════════
    if tipo == "llamado_atencion":
        from utils.documentos_premium import generar_llamado_atencion_premium
        return generar_llamado_atencion_premium(
            empleado_raw=empleado, empresa_raw=datos_empresa,
            ruta=ruta_salida, config=datos_extra or {},
            disenio=disenio, usar_marca_agua=False,
        )
    if tipo == "citacion_descargos":
        from utils.documentos_premium import generar_citacion_descargos_premium
        return generar_citacion_descargos_premium(
            empleado_raw=empleado, empresa_raw=datos_empresa,
            ruta=ruta_salida, config=datos_extra or {},
            disenio=disenio, usar_marca_agua=False,
        )
    ...
    # Los documentos disciplinarios y de citación NO llevan marca de agua
    # (podría interpretarse como que el documento no es válido / es borrador)
    DOCS_SIN_MARCA_AGUA = {
        "llamado_atencion", "citacion_descargos", "citacion",
        "memorando_disciplinario", "descargos", "sanciones",
    }
    if tipo in DOCS_SIN_MARCA_AGUA:
